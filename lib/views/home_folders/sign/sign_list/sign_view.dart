import 'package:MWPX/blocs/home_folders/sign/sign_list/sign_list_bloc.dart';
...
class SignView extends StatelessWidget {
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: appBar,
      body: BlocProvider(
        create: (BuildContext context) => SignListBloc(
          sharedPrefsService: sharedPrefsService,
...
class SignViewBody extends StatefulWidget {
  @override
  _SignViewBodyState createState() => _SignViewBodyState();
}
...
class _SignViewBodyState extends State<SignViewBody> {
  ...
  Widget build(BuildContext context) {
    ...
    buttonBar.configureButtonBar(Constants.viewNameSign);

    List<SignListItem> _documentList = [];
    ...
    for (int i = 0; i < 100; i++) {
      ...
      // signItem.regDATE = DateTime.now().add(new Duration(days: -1 * i));
      signItem.rcvdDT = DateTime.now().add(new Duration(days: -1 * i));
      // signItem.mainAuthor = "Иванов$i Степан$i Петрович$i";
      signItem.documentTypeText = 'Приказ';
      signItem.dokvr = "00$i";
      signItem.doktl = "000$i";
      signItem.wfItem = "02$i";
      signItem.logsys = 'logsys';

      _documentList.add(signItem);
    }

    DecisionDataSource _decisionListSource =
        new DecisionDataSource(_documentList);

    return BlocConsumer<SignListBloc, SignListState>(
      listener: (context, state) {
        if (state is SignListFailure) {
          Dialogs.errorDialog(
            context: context,
            content: Text('${state.error}'),
          );
        }
        if (state is SignListSortInit) {
        ...
        return Column(
          ...
          children: <Widget>[
            ...
            Padding(
              ...
              child: FlatButton(
                  onPressed: () {
                    BlocProvider.of<SignListBloc>(context).add(
                      SortDataSave(
...
class DecisionDataSource extends DataGridSource {
  ...
  DecisionDataSource(List<SignListItem> employees) {
